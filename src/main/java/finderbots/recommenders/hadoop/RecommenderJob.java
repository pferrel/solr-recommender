...
public final class RecommenderJob extends AbstractJob {
  ...
  public static final String BOOLEAN_DATA = "booleanData";
    ...
    private static final String SIMS_MATRIX_PATH = "sims";
  ...
  public int run(String[] args) throws Exception {
    ...
    if (shouldRunNextPhase(parsedArgs, currentPhase)) {
      ...
      Job prePartialMultiply2 = prepareJob(new Path(prepPath, PreparePreferenceMatrixJob.USER_VECTORS),
                                           ...
                                           SequenceFileOutputFormat.class);
      if (usersFile != null) {
        prePartialMultiply2.getConfiguration().set(USERS_FILE, usersFile);
      }
      prePartialMultiply2.getConfiguration().setInt(MAX_PREFS_PER_USER_CONSIDERED,
              maxPrefsPerUser);
    ...
    if (shouldRunNextPhase(parsedArgs, currentPhase)) {
      ...
      // Pat: changed the output type to SequenceFileOutputFormat.class from TextOutputFormat.class
      // Note: need to check here for the type of the file we are reading recs from. the XRecommender
      // writes keys = Long, values = VectorWriteable
      // the mahout mapreduce recommender writes:
      // Key class: class org.apache.mahout.math.VarLongWritable Value Class: class org.apache.mahout.cf.taste.hadoop.RecommendedItemsWritable
      //
